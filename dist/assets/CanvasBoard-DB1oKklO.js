...
function NodeItem({ node, selected, snapToGrid, activeTool, taskProgress, onPointerDownAction, onMove, onMoveEnd, onResize, onResizeEnd, scale, onOpenRightPanel, onOpenSettings, onToggleComplete, onDelete, onAddChild, onTextChange, onEdgeDragStart, onDropResource }) {
	...
	if (node.type === "FloatingText") return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: cn("absolute top-0 left-0 pointer-events-auto min-w-[50px] p-2 z-10 group outline-none", selected && "ring-2 ring-purple-500/60 shadow-lg rounded-md", isDragging ? "opacity-90 scale-[1.02] z-50 cursor-grabbing" : isPanMode ? "cursor-grab" : isSelectMode ? isEditingText ? "cursor-text" : "cursor-pointer" : ""),
		style: {
	...
	if ([
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: cn("absolute top-0 left-0 pointer-events-auto flex items-center justify-center z-10 group text-slate-700", selected && "shadow-md", isDragging || isResizing ? "opacity-90 z-50 shadow-lg cursor-grabbing scale-[1.02]" : isPanMode ? "cursor-grab" : isSelectMode ? "cursor-pointer" : ""),
			style: {
				transform: `translate3d(${node.x}px, ${node.y}px, 0)`,
				width: w,
				height: h,
	...
	if (node.type === "Text") return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: cn("absolute top-0 left-0 pointer-events-auto min-w-[150px] max-w-[400px] p-4 bg-yellow-50/90 backdrop-blur-sm rounded-xl shadow-sm border border-yellow-200 text-slate-800 z-10 group", selected && "ring-2 ring-purple-500/60 shadow-md", isDragging ? "opacity-90 scale-[1.02] z-50 cursor-grabbing shadow-lg" : isPanMode ? "cursor-grab" : isSelectMode ? isEditingText ? "cursor-text" : "cursor-pointer" : ""),
		style: {
			transform: `translate3d(${node.x}px, ${node.y}px, 0)`,
			transition: isDragging ? "none" : "transform 0.15s cubic-bezier(0.2, 0, 0, 1)"
		},
	...
	if (node.type === "Image") return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: cn("absolute top-0 left-0 pointer-events-auto w-[300px] rounded-2xl shadow-sm border border-slate-200 bg-white z-10 overflow-hidden group", selected && "ring-4 ring-purple-500/40 border-purple-500/50 shadow-md", isDragging ? "opacity-90 scale-[1.02] z-50 cursor-grabbing shadow-lg" : isPanMode ? "cursor-grab" : isSelectMode ? "cursor-pointer" : ""),
		style: {
			transform: `translate3d(${node.x}px, ${node.y}px, 0)`,
			transition: isDragging ? "none" : "transform 0.15s cubic-bezier(0.2, 0, 0, 1)"
		},
...
function CanvasBoard({ funnel, onChange, hideHeader, onBack }) {
	...
	const handleEdgeDragStart = (nodeId, e) => {
		...
		const onUp = (ev) => {
			...
			if (targetNodeEl) {
				...
				if (targetId && targetId !== nodeId && !funnel.edges.some((edge) => edge.source === nodeId && edge.target === targetId)) onChange({
					...funnel,
					edges: [...funnel.edges, {
						id: generateId("e"),
						source: nodeId,
	...
	const updateNodeStyle = (updates) => {
		if (selectedNodes.length > 0) onChange({
			...funnel,
			nodes: funnel.nodes.map((n) => selectedNodes.includes(n.id) ? {
				...n,
				style: {
						...
						children: funnel.nodes.map((n) => {
							...
							return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(NodeItem, {
								...
								onMoveEnd: (dx, dy) => {
									...
									onChange({
										...funnel,
										nodes: funnel.nodes.map((node) => selectedNodes.includes(node.id) ? {
											...node,
											x: node.x + dx,
											y: node.y + dy
										} : node)
...
//# sourceMappingURL=CanvasBoard-DB1oKklO.js.map